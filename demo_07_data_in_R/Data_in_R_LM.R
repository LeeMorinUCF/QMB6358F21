...
# Show the row names of the data.
rownames(cars_df)
...
cars_df[2:5,'dist']
cars_df[c("2", "5"),'dist'] # Not very common.
cars_df$dist[2:5]
...
iris_area_df <- data.frame(Species_2 = iris_df$Species,
...
# cbind() is like the paste command in UNIX.



# Check if species match.
table(iris_wide_df$Species, iris_wide_df$Species_2)


# What happens?
iris_and_cars <- cbind(iris_df, cars_df)
summary(iris_and_cars)
# It copies the cars dataset 3 times.
...
# rbind() is like the cat command in UNIX.
...
# Interesting idea:
# Loop on a list of file names.

list.files(path = "~/Teaching/QMB6358_Fall_2020/GitRepos/QMB6358F20/demo_08_data_in_R",
           pattern = "iris")
...
# Loop on these files.
files_with_a <- list.files(path = "~/Teaching/QMB6358_Fall_2020/GitRepos/QMB6358F20/demo_08_data_in_R",
             pattern = "a")

for (file_name in files_with_a) {
...
for (file_num in 1:length(files_with_a)) {

  file_name <- files_with_a[file_num]
  print(file_name)
...
for (file_num in 1:7) {
  ...
  file_name <- paste('my_file_', file_num, '.csv', sep = "")
  # file_name <- paste('my_file_', file_num, '.csv', sep = "whatever")
  print(file_name)
